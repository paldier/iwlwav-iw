...
/* Converts hexadecimal string src to binary. Tolerates odd number of input
   symbols. Returns length of resulting binary pointed by dst. */
int hex2bin2(char *dst, size_t output_size, const char *src, size_t input_size)
{
	int returnlen = output_size;
	int hilo;

	if (output_size == 0 || input_size == 0)
		return 0;
	/* if count is odd, first symbol contains the whole byte */
	if (input_size % 2) {
		hilo = hex2num(*src++);
		if (hilo < 0)
			return -1;
...
void print_ht_tx_bf(__u32 cap)
{

#define PRINT_HT_TX_BF_CAP(_cond, _str) \
	do { \
		if (_cond) \
			printf("\t\t\t" _str "\n"); \
	} while (0)

	printf("\t\tHT TX_BF Capabilities: 0x%04x\n", cap);

	PRINT_HT_TX_BF_CAP((cap & BIT(0)), "IMPL-TXBF-RX");
	PRINT_HT_TX_BF_CAP((cap & BIT(1)), "RX-STAG-SND");
	PRINT_HT_TX_BF_CAP((cap & BIT(2)), "TX-STAG-SND");
	PRINT_HT_TX_BF_CAP((cap & BIT(3)), "RX-NDP");
	PRINT_HT_TX_BF_CAP((cap & BIT(4)), "TX-NDP");
	PRINT_HT_TX_BF_CAP((cap & BIT(5)), "IMPL-TXBF");
	PRINT_HT_TX_BF_CAP(((cap >> 6) & 0x3) == 1, "CALIB-RESP");
	PRINT_HT_TX_BF_CAP(((cap >> 6) & 0x3) == 3, "CALIB-RESP+REQ");
	PRINT_HT_TX_BF_CAP((cap & BIT(8)), "EXPL-CSI-TXBF");
	PRINT_HT_TX_BF_CAP((cap & BIT(9)), "EXPL-NONCOMPR-STEER");
	PRINT_HT_TX_BF_CAP((cap & BIT(10)), "EXPL-COMPR-STEER");

	PRINT_HT_TX_BF_CAP(((cap >> 11) & 0x3) == 1, "EXPL-TXBF-CSI-FBACK-DELAYED");
	PRINT_HT_TX_BF_CAP(((cap >> 11) & 0x3) == 2, "EXPL-TXBF-CSI-FBACK-IMM");
	PRINT_HT_TX_BF_CAP(((cap >> 11) & 0x3) == 3, "EXPL-TXBF-CSI-FBACK-DELAYED+IMM");

	PRINT_HT_TX_BF_CAP(((cap >> 13) & 0x3) == 1, "EXPL-NONCOMPR-FB-FBACK-DELAYED");
	PRINT_HT_TX_BF_CAP(((cap >> 13) & 0x3) == 2, "EXPL-NONCOMPR-FB-FBACK-IMM");
	PRINT_HT_TX_BF_CAP(((cap >> 13) & 0x3) == 3, "EXPL-NONCOMPR-FB-FBACK-DELAYED+IMM");

	PRINT_HT_TX_BF_CAP(((cap >> 15) & 0x3) == 1, "EXPL-COMPR-FB-FBACK-DELAYED");
	PRINT_HT_TX_BF_CAP(((cap >> 15) & 0x3) == 2, "EXPL-COMPR-FB-FBACK-IMM");
	PRINT_HT_TX_BF_CAP(((cap >> 15) & 0x3) == 3, "EXPL-COMPR-FB-FBACK-DELAYED+IMM");

	PRINT_HT_TX_BF_CAP(((cap >> 17) & 0x3) == 1, "MIN-GROUP-12");
	PRINT_HT_TX_BF_CAP(((cap >> 17) & 0x3) == 2, "MIN-GROUP-14");
	PRINT_HT_TX_BF_CAP(((cap >> 17) & 0x3) == 3, "MIN-GROUP-124");

	PRINT_HT_TX_BF_CAP(((cap >> 19) & 0x3) == 0, "CSI-BF-ANT-1");
	PRINT_HT_TX_BF_CAP(((cap >> 19) & 0x3) == 1, "CSI-BF-ANT-2");
	PRINT_HT_TX_BF_CAP(((cap >> 19) & 0x3) == 2, "CSI-BF-ANT-3");
	PRINT_HT_TX_BF_CAP(((cap >> 19) & 0x3) == 3, "CSI-BF-ANT-4");

	PRINT_HT_TX_BF_CAP(((cap >> 21) & 0x3) == 0, "NONCOMPS-BF-ANT-1");
	PRINT_HT_TX_BF_CAP(((cap >> 21) & 0x3) == 1, "NONCOMPS-BF-ANT-2");
	PRINT_HT_TX_BF_CAP(((cap >> 21) & 0x3) == 2, "NONCOMPS-BF-ANT-3");
	PRINT_HT_TX_BF_CAP(((cap >> 21) & 0x3) == 3, "NONCOMPS-BF-ANT-4");

	PRINT_HT_TX_BF_CAP(((cap >> 23) & 0x3) == 0, "COMPS-BF-ANT-1");
	PRINT_HT_TX_BF_CAP(((cap >> 23) & 0x3) == 1, "COMPS-BF-ANT-2");
	PRINT_HT_TX_BF_CAP(((cap >> 23) & 0x3) == 2, "COMPS-BF-ANT-3");
	PRINT_HT_TX_BF_CAP(((cap >> 23) & 0x3) == 3, "COMPS-BF-ANT-4");

	PRINT_HT_TX_BF_CAP(((cap >> 25) & 0x3) == 0, "CSI-MAX-ROWS-BF-1");
	PRINT_HT_TX_BF_CAP(((cap >> 25) & 0x3) == 1, "CSI-MAX-ROWS-BF-2");
	PRINT_HT_TX_BF_CAP(((cap >> 25) & 0x3) == 2, "CSI-MAX-ROWS-BF-3");
	PRINT_HT_TX_BF_CAP(((cap >> 25) & 0x3) == 3, "CSI-MAX-ROWS-BF-4");

	PRINT_HT_TX_BF_CAP(((cap >> 27) & 0x3) == 0, "CHE-SPACE-TIME-STR-1");
	PRINT_HT_TX_BF_CAP(((cap >> 27) & 0x3) == 1, "CHE-SPACE-TIME-STR-2");
	PRINT_HT_TX_BF_CAP(((cap >> 27) & 0x3) == 2, "CHE-SPACE-TIME-STR-3");
	PRINT_HT_TX_BF_CAP(((cap >> 27) & 0x3) == 3, "CHE-SPACE-TIME-STR-4");
}
